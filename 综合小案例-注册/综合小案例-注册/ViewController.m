...
- (IBAction)rigiste:(NSButton *)sender {
    ...
    if([_username.stringValue isEqualToString:@""]){
        ...
        [self showDialog:@"温馨提示" :@"用户名不能为空" :NSAlertStyleWarning];
    ...
    if([_password.stringValue isEqualToString:@""]){
       ...
       // NSLog(@"密码不能为空");
        ...
        [self showDialog:@"温馨提示" :@"密码不能为空" :NSAlertStyleWarning];
    ...
    if([_repassword.stringValue isEqualToString:@""]){
        ...
        [self showDialog:@"温馨提示" :@"确认密码不能为空" :NSAlertStyleWarning];
    ...
    if(![_password.stringValue isEqualToString:_repassword.stringValue]){
        ...
        [self showDialog:@"温馨提示" :@"密码和确认密码不一致" :NSAlertStyleWarning];
    ...
    [self showDialog:@"注册信息" :[NSString stringWithFormat:@"用户名为：%@， 密码为：%@， 性别为：%@， 兴趣爱好：%@", _username.stringValue, _password.stringValue, _sex, _interests] :NSAlertStyleInformational];
    
    
}

-(void)showDialog:(NSString *)title :(NSString *)message :(int)type{
    ...
    [alertDialog addButtonWithTitle:@"确定"];
